...
Require deterministic five-requirement batches per lane, report-bound `GB-1-####`/`GB-2-####` identities, and one request per unresolved contested baseline requirement per lane. Every request contains the exact typed stable-baseline projection, exact revised report bytes/hash, exact admitted `generation_validation` object (receipt/report/bundle/coverage hashes, completed status, and native validation booleans), exact report-passage allowlist, exact v2.2 rubric/scoring-contract fingerprint, and no historical v2.2 grade, disposition, reason code, response, report label, or candidate identifier. Binding generation validation in every operation lets replay detect an admitted-input hash change before the safety phase instead of leaving those fields unused. The two lanes receive identical legal/report evidence but distinct controller lane IDs and request fingerprints.
...
Require rows for every origin; stable ordering by origin priority, baseline canonical order, subject ID, finding kind, and fingerprint; contiguous `GAP-####`; exact report passages; deterministic follow-up; and one row per safety finding. Mutation-test hidden gaps, unsupported assertions, contradictions, client-fact dependencies, misleading currentness/authority/language, missing follow-up, missing rationale component, generic rationale, evidence swap, false `resolved`, missing critical prominent disclosure, unsafe owner, a replayed safety finding that establishes a completeness/certainty claim contradicting the matrix, and outcome-determinative contest. Known lexical completeness phrases may remain deterministic defense in depth, but tests must not imply that a finite phrase inventory proves semantic absence.
...
5. If every gap is visible/actionable, critical disclosure/ownership is safe, the mandatory replayed safety lanes/referees do not establish a completeness or certainty claim that contradicts the matrix, and no blocker exists, return `REVIEW_READY_WITH_GAPS` even when the fresh strict-equivalent disposition is `FAIL` or substantive `INCONCLUSIVE`. Do not attempt to prove semantic absence with a finite paraphrase list, and do not make every disclosed gap nondeliverable.
...
Replay must reconstruct every fresh grade and safety request from `readiness-input.json`, the verified gradeable-baseline projection, rubric/scoring-contract bytes, prior accepted responses, and controller rules; recompile both grader aggregates, strict-equivalent result, safety aggregate, matrices, tier, and handoff; compare exact bytes; and verify the exact relative inventory and artifact hashes. In particular, every Task 3 request binds the exact admitted generation-validation object, so changing its receipt, bundle, or coverage hashes while merely resealing the input and outer manifest fails against the stored request bytes even before safety review. This is a semantic-replay guarantee, not a claim that unkeyed hashes inside a standalone directory can identify the original run after an attacker replaces every input, request, response, derived artifact, and manifest with one fully coordinated valid graph; that stronger provenance claim requires an external immutable root, signature, or original-source reread. `readiness-verification.json` binds a deterministic pre-manifest graph fingerprint and allowlisted check booleans; the terminal manifest binds its exact bytes without a circular manifest-root reference.
...
For each exact pending grade or safety request, create attempt 1 with no clarification codes. On `NeedsReadinessClarificationV1`, issue attempt 2 in a fresh context with only allowlisted reason codes, never rejected bytes. A second refusal returns a pause outcome with the original request still pending. Unfavorable grades and safety findings are substantive and are accepted/replayed; never retry either to obtain a better strict-equivalent disposition or tier. For an internally driven accepted `fresh_context` call, hash the exact bounded context token and persist that fingerprint in the immutable `ReadinessCallRecordV1`; the manifest/root therefore authenticates freshness history across process restart. Externally guarded accepted responses and explicit scripted fixtures keep `context_token_fingerprint=None`. A mutable response-control ledger may reserve tokens before provider execution, but it is not the trust anchor and missing/truncated/resealed ledger history cannot erase an immutable accepted-call token fingerprint.

- [ ] **Step 5: Add public-safe telemetry and freshness assertions**

Telemetry may contain protocol/compiler/scoring fingerprints, operation, grader-or-safety lane, batch/contested/dispute class, attempt, normalization/clarification codes, pause/resume counts, and no private text/paths/historical disposition/report labels. Tests must instantiate a new evaluator context token for every grade fragment, contested grade, safety lane, referee, and repair and reject adapter reuse that claims `judge_isolation != "fresh_context"` outside scripted fixtures. Tests must also delete, truncate, and canonically reseal the mutable token ledger and prove it cannot reset or forge freshness against immutable accepted-call fingerprints.
...
The concrete output uses the run's real 64-hex fingerprint; the repeated `a` value is a valid synthetic fixture digest. Pending status sets the fresh strict-equivalent and readiness fields to `null`, retains only already-bound historical cross-check metadata, exposes only operation/lane/batch/contested/dispute class, and exits `0`. A second mechanical refusal is deliberately write-free, so its verified engine-pause state exists only in the live `continue_readiness_v1()` outcome: that host-controller outcome sets `engine_paused=true` and exits `6`, while a later stateless status or verify command sees the unchanged valid pending run, reports no persisted pause state, and exits `0`. The locked five-command CLI has no evaluator-driving run/resume command and must not infer pause from ordinary pending state or trust a mutable response-control ledger as pause authority. Verification returns `ok`, manifest/root/result/scoring fingerprints, fresh strict-equivalent disposition, optional historical v2.2 disposition/cross-check status, delivery readiness, and allowlisted issue codes only. Human output prints `Baseline-locked strict-equivalent: PASS`, `Historical Protocol 2.2 strict disposition: FAIL (cross-check differs)`, and `Delivery readiness: REVIEW_READY_WITH_GAPS` on separate lines; when no history is supplied it prints `Historical Protocol 2.2 strict disposition: not supplied`.

- [ ] **Step 4: Implement command dispatch and exit codes**

Initialization/input errors—including supplying only one historical option—exit `2` without creating a run. Fresh verified substantive strict-equivalent/readiness inconclusive without a safe tier exits `3`. Verified `NOT_DELIVERABLE` exits `4`. Integrity or unsupported secure storage exits `5`. A live workflow-driver pause exits `6`; the stateless five-command CLI does not reconstruct that ephemeral outcome. Verified `HIGH_ASSURANCE` and `REVIEW_READY_WITH_GAPS` exit `0`; historical v2.2 disagreement never changes the exit.
